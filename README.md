...
The goal of this project is to search for and explore close approaches of near-Earth objects (NEOs), using data from NASA/JPL's Center for Near Earth Object Studies.

## Overview

At a high-level, a command-line tool to inspect and query a dataset of NEOs and their close approaches to Earth will be created.

Data is stored in CSV and JSON file, it will get converted into structured Python objects, on which filtering operations will be performed, limiting the size of the result set, and writing the results to a file in a structured format, such as CSV or JSON.

When complete, this tool will allow inspecting the properties of the near-Earth objects in the data set and query the data set of close approaches to Earth using any combination of the following filters:
...
In this CSV file, the first row is a header, containing names for each of the columns. Each subsequent row represents a single NEO. There are too many columns to understand fully, so we'll focus on just a few of them:
